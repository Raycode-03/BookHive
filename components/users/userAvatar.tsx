...
export function UserAvatar({ avatar, email, size = 32, className = "" }: UserAvatarProps) {
  ...
  if (avatar && (avatar.startsWith('http://') || avatar.startsWith('https://'))) {
    return (
      <Image
        ...
        className={` border object-cover ${className}`}
  ...
  return (
    <div 
      className={`${getBackgroundColor(displayText)}  border flex items-center justify-center text-white font-semibold ${className}`}
