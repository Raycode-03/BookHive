...
export async function GET() {
  try {
    ...
    const activeLoans = await db.collection("borrows").countDocuments({
      userId,
      status: "active",
    });

    const overdueLoans = await db.collection("borrows").countDocuments({
      userId,
      status: "active",
      returnDate: { $lt: today },
    });

    const activeReserves = await db.collection("reserves").countDocuments({
      userId,
      status: "active",
    });
  ...
  const activeLoansList = await db.collection("borrows").aggregate([
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        ActiveReservesCount: activeReserves,
